...
int main(){
    ...
    insert_list_m_elements(&List1,numb);

}

void insert_list_m_elements(ListPointer *List, int n){

    int i,thesi,plithos,j;
    ListElementType AnItem;
    ListPointer PredPtr=NULL,TmpPtr;


    for(i=0;i<n;i++){
        printf("DWSE TON ARI8MO GIA EISAGWGH STH ARXH THS LISTAS:");
        scanf("%d",&AnItem);
        LinkedInsert(List,AnItem,PredPtr);
    }
    ...
    scanf("%d",&thesi);
    if(thesi>n || n<1)printf("ERROR\n");
    else{
        printf("DWSE TO PLITHOS TWN STOIXEIWN POU THA EISAX8OYN:");
        scanf("%d",&plithos);
        for(i=0;i<plithos;i++){
